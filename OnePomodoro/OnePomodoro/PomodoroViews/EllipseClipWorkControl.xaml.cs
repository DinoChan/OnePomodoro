...
namespace OnePomodoro.PomodoroViews
{
    public sealed partial class EllipseClipWorkControl : PomodoroView
    {
        ...
        {
            Section1.Content = (ViewModel.RemainingPomodoroTime.Minutes / 10).ToString();
            Section2.Content = (ViewModel.RemainingPomodoroTime.Minutes % 10).ToString();
            Section3.Content = (ViewModel.RemainingPomodoroTime.Seconds / 10).ToString();
            Section4.Content = (ViewModel.RemainingPomodoroTime.Seconds % 10).ToString();
        }
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                StartClipAnimation(_compositionEllipseGeometries[i], TimeSpan.FromSeconds(1 + i));
            }

            for (int i = 0; i < _timeVisuals.Count; i++)
            {
                StartOffsetAnimation(_timeVisuals[i], TimeSpan.FromSeconds(1 + i));
            }
